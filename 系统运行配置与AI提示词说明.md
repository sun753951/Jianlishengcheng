...
说明：默认基础模型为 `uer/gpt2-chinese-cluecorpussmall`，用于本地快速验证中文 Prompt 训练流程。若本机下载或训练速度无法接受，可临时改用 `sshleifer/tiny-gpt2` 验证脚本流程，但该 tiny 模型生成质量很弱，不适合作为最终效果展示。若需要更好效果，应在云 GPU 上换用更大的中文或多语言模型，并提高训练样本数和训练步数。

本机已完成一次轻量训练和评估，记录见 `训练与评估记录.md`。结论是：本地小样本模型能学习简历结构，但生成质量和 2 秒指标尚不稳定。因此后端保留模型质量校验，模型输出不合格时自动回退到规则模板，保证演示流程稳定。
